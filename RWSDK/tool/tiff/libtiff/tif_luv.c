...
LogL16Decode(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	int cc, rc;
        ...
        RWFUNCTION(RWSTRING("LogL16Decode"));
...
LogLuvDecode24(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvDecode24"));
        RWASSERT(s == 0);
	RWASSERT(sp != NULL);
...
LogLuvDecode32(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvDecode32"));
...
LogLuvDecodeStrip(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvDecodeStrip"));
        RWASSERT(cc%rowlen == 0);
...
LogLuvDecodeTile(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvDecodeTile"));
        RWASSERT(cc%rowlen == 0);
	while (cc && (*tif->tif_decoderow)(tif, bp, rowlen, s))
		bp += rowlen, cc -= rowlen;
	RWRETURN (cc == 0);
...
LogL16Encode(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogL16Encode"));
        RWASSERT(s == 0);
	RWASSERT(sp != NULL);
...
LogLuvEncode24(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvEncode24"));
        RWASSERT(s == 0);
	RWASSERT(sp != NULL);
	npixels = cc / sp->pixel_size;
...
LogLuvEncode32(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvEncode32"));
        RWASSERT(s == 0);
	RWASSERT(sp != NULL);
...
LogLuvEncodeStrip(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvEncodeStrip"));
        RWASSERT(cc%rowlen == 0);
...
LogLuvEncodeTile(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("LogLuvEncodeTile"));
        RWASSERT(cc%rowlen == 0);
	while (cc && (*tif->tif_encoderow)(tif, bp, rowlen, s) == 0)
		bp += rowlen, cc -= rowlen;
	RWRETURN (cc == 0);
...
pix16fromY(double Y)
{
	...
	if (Y <= -1.84467e19)
		return (0xffff);
	if (Y > 5.43571e-20)
		return IntFromFloatMacro((float)(256.*(log2(Y) + 64.)));
	if (Y < -5.43571e-20)
		return (~0x7fff | 
                        IntFromFloatMacro((float)(256.*(log2(-Y) + 64.))));
	return (0);
}
...
L16toGry(LogLuvState* sp, tidata_t op, int n)
{
	...
	while (n-- > 0) {
		...
		*gp++ = (Y <= 0.) ? 
                    ...
                    IntFromFloatMacro((float)(256.*sqrtf(Y)));
#else /* (defined(RW_USE_SPF)) */
                    IntFromFloatMacro((float)(256.*sqrt(Y)));
...
XYZtoRGB24(float xyz[3], uint8 rgb[3])
{
	...
	rgb[0] = (r <= 0.) ? 
            ...
            (r >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrtf(r)));
	rgb[1] = (g <= 0.) ? 
            0 : 
            (g >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrtf(g)));
	rgb[2] = (b <= 0.) ? 
            0 : 
            (b >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrtf(b)));
#else /* (defined(RW_USE_SPF)) */
    rgb[0] = (r <= 0.) ? 
            0 : 
            (r >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrt(r)));
	rgb[1] = (g <= 0.) ? 
            0 : 
            (g >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrt(g)));
	rgb[2] = (b <= 0.) ? 
            0 : 
            (b >= 1.) ? 
            255 : 
            IntFromFloatMacro((float)(256.*sqrt(b)));
#endif /* (defined(RW_USE_SPF)) */
}
...
uv_encode(double u, double v)		/* encode (u',v') coordinates */
{
	...
	if (v < UV_VSTART)
		return(-1);
	vi = IntFromFloatMacro((float)((v - UV_VSTART)*(1./UV_SQSIZ)));
	if (vi >= UV_NVS)
		return(-1);
	if (u < uv_row[vi].ustart)
		return(-1);
	ui = IntFromFloatMacro((float)((u - uv_row[vi].ustart)*(1./UV_SQSIZ)));
	if (ui >= uv_row[vi].nus)
		return(-1);
...
pix24fromXYZ(float XYZ[3])
{
	...
	if (L >= 16.)
		...
		Le = IntFromFloatMacro((float)(64.*(log2(L) + 12.)));
					/* encode color */
...
Luv24toLuv48(LogLuvState* sp, tidata_t op, int n)
{
	...
	while (n-- > 0) {
		...
		*luv3++ = (int16)IntFromFloatMacro((float)(u * (1L<<15)));
		*luv3++ = (int16)IntFromFloatMacro((float)(v * (1L<<15)));
		luv++;
	}
}
...
pix32fromXYZ(float XYZ[3])
{
	...
	if (u <= 0.) ue = 0;
	else ue = (unsigned int)IntFromFloatMacro((float)(UVSCALE * u));
	if (ue > 255) ue = 255;
	if (v <= 0.) ve = 0;
	else ve = (unsigned int)IntFromFloatMacro((float)(UVSCALE * v));
	if (ve > 255) ve = 255;
					/* combine encodings */
...
LogL16InitState(TIFF* tif)
{
	...
	RWFUNCTION(RWSTRING("LogL16InitState"));
        RWASSERT(sp != NULL);
	RWASSERT(td->td_photometric == PHOTOMETRIC_LOGL);
...
LogLuvInitState(TIFF* tif)
{
	...
	RWFUNCTION(RWSTRING("LogLuvInitState"));
        RWASSERT(sp != NULL);
	RWASSERT(td->td_photometric == PHOTOMETRIC_LOGLUV);
...
TIFFInitSGILog(TIFF* tif, int scheme)
{
	...
	RWFUNCTION(RWSTRING("TIFFInitSGILog"));
        RWASSERT(scheme == COMPRESSION_SGILOG24 || scheme == COMPRESSION_SGILOG);
