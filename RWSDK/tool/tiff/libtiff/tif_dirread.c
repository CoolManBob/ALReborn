/* $Header: //RenderWare/RW37Active/rwsdk/tool/tiff/libtiff/tif_dirread.c#1 $ */
...
TIFFReadDirectory(TIFF* tif)
{
	...
	if (!isMapped(tif)) {
		 ...
		 */
        if (!ReadOK(tif, &nextdiroff, sizeof (uint32))) {
            goto bad;
        }
	} else {
...
/* This code causes ee-gcc to bomb with an internal compiler error when optimizations
are turned on - luckily Renderware doesn't seem to use whatever the result is,
even though this code gets executed while loading TIFFs! */
#if !(defined(__GNUC__) && defined(SKY))
			  ok = (fip->field_passcount ?
			      TIFFSetField(tif, dp->tdir_tag, 1, &v)
			    : TIFFSetField(tif, dp->tdir_tag, v));
#endif
