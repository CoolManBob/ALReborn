/* $Header: //RenderWare/RW37Active/rwsdk/tool/tiff/libtiff/tif_predict.c#1 $ */
...
PredictorDecodeRow(TIFF* tif, tidata_t op0, tsize_t occ0, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("PredictorDecodeRow"));
        RWASSERT(sp != NULL);
	RWASSERT(sp->coderow != NULL);
	RWASSERT(sp->pfunc != NULL);
...
PredictorDecodeTile(TIFF* tif, tidata_t op0, tsize_t occ0, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("PredictorDecodeTile"));
        RWASSERT(sp != NULL);
	RWASSERT(sp->codetile != NULL);
...
PredictorEncodeRow(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("PredictorEncodeRow"));
        RWASSERT(sp != NULL);
	RWASSERT(sp->pfunc != NULL);
	RWASSERT(sp->coderow != NULL);
/* XXX horizontal differencing alters user's data XXX */
	(*sp->pfunc)(tif, bp, cc);
	RWRETURN ((*sp->coderow)(tif, bp, cc, s));
}
...
PredictorEncodeTile(TIFF* tif, tidata_t bp0, tsize_t cc0, tsample_t s)
{
	...
	RWFUNCTION(RWSTRING("PredictorEncodeTile"));
        RWASSERT(sp != NULL);
	RWASSERT(sp->pfunc != NULL);
	RWASSERT(sp->codetile != NULL);
	rowsize = sp->rowsize;
	...
	RWRETURN ((*sp->codetile)(tif, bp0, cc0, s));
}
