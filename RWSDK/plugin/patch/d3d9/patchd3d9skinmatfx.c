...
/*****************************************************************************
 NodeDefinitionGetD3D9PatchSkinMatFXAtomicInstance

 Returns a pointer to a node to instance a skin patch matfx atomic in a form
 suitable for rendering by a D3D9 pipeline.
...
*/
static RxNodeDefinition   *
NodeDefinitionGetD3D9PatchSkinMatFXAtomicInstance(void)
{
    static RwChar _PatchAtomic_csl[] = RWSTRING("PatchSkinMatFXAtomic.csl");
    ...
    RWFUNCTION(RWSTRING("NodeDefinitionGetD3D9PatchSkinMatFXAtomicInstance"));
...
_rpPatchPipelinesAttach(RpAtomic *atomic, RpPatchType type)
{
    ...
    if(type == rpNAPATCHTYPE)
    {
        if (matfx)
        {
            if (skinned)
            {
                pipeline = _rpPatchGlobals.platform.pipelines[rpPATCHD3D9PIPELINESKINMATFX];
            }
            else
            {
