...
RwD3D9RasterStreamReadDDS(RwStream *stream)
{
    ...
    if (DDS_FOURCC == header.ddspf.dwFlags)
    {
		...
		if (volumeTexture)
		{
            convert = (_rwD3D9CheckValidVolumeTextureFormat(header.ddspf.dwFourCC) ==
                       FALSE);
            
			if (convert)
            {
                RwDebugSendMessage(rwDEBUGMESSAGE, "RwD3D9RasterStreamReadDDS",
                                   "Volume texture format not supported");
                RWRETURN(NULL);
            }
		}
		else
        ...
        if (!convert)
        {
			...
			if (volumeTexture)
			{
                if (_RwD3D9DeviceCaps.VolumeTextureFilterCaps & (D3DPTFILTERCAPS_MIPFPOINT | D3DPTFILTERCAPS_MIPFLINEAR))
                {
                ...
                else
                {
                    ...
                    {
                        RwDebugSendMessage(rwDEBUGMESSAGE, "RwD3D9RasterStreamReadDDS",
                                          "Volume texture mip-mapping not supported");
                    }
                    #endif
                ...
                hr = DXCHECK(IDirect3DDevice9_CreateVolumeTexture(_RwD3DDevice,
                                                            header.dwWidth,
															header.dwHeight,
															header.dwDepth,
                                                            levels,
                                                            ...
                                                            D3DPOOL_MANAGED,
                                                            (LPDIRECT3DVOLUMETEXTURE9 *)&(rasExt->texture),
                                                            NULL));
			}
			else
			//@} Jaewon
    ...
    else if (DDS_RGB == header.ddspf.dwFlags)
    {
		...
		if (volumeTexture)
		{
            ...
            if (header.ddspf.dwRGBBitCount == 24)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_R8G8B8))
                {
            ...
            else if (0x00ff0000 == header.ddspf.dwRBitMask)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_X8R8G8B8))
                {
                    rasFormat = rwRASTERFORMAT888;
                }
                else
            ...
            else if (0x0000f800 == header.ddspf.dwRBitMask)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_R5G6B5))
                {
            ...
            if (!convert)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        hr = DXCHECK(
                            IDirect3DDevice9_CreateVolumeTexture(_RwD3DDevice,
                                                               header.dwWidth,
															   header.dwHeight,
															   header.dwDepth,
                                                               levels,
    ...
    else if (DDS_RGBA == header.ddspf.dwFlags)
    {
		...
		if (volumeTexture)
		{
            ...
            if (0x00ff0000 == header.ddspf.dwRBitMask)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_A8R8G8B8))
                {
            ...
            else if (0x00007c00 == header.ddspf.dwRBitMask)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_A1R5G5B5))
                {
            ...
            else if (0x00000f00 == header.ddspf.dwRBitMask)
            {
                ...
                if (_rwD3D9CheckValidVolumeTextureFormat(D3DFMT_A4R4G4B4))
                {
        ...
        do
        {
            ...
            for (i = 0; i < numMipLevels; ++i)
            {
                ...
                else
                {
                    ...
                    /* Get the volume descriptor to find the size */
                    IDirect3DVolumeTexture9_GetLevelDesc((LPDIRECT3DVOLUMETEXTURE9)(rasExt->texture), i, &volumeDesc);
                    ...
                    {
                        size = volumeDesc.Depth * (raster->stride);
