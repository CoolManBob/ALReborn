...
RxStateCache RenderStateCache[MAX_RENDERSTATE_CACHE];

RwUInt32 RenderStateD3D[MAX_RENDERSTATE_CACHE];
...
RxStateCache TextureStageStateCache[RWD3D9_MAX_TEXTURE_STAGES][MAX_TEXTURESTAGESTATE_CACHE];

RwUInt32 TextureStageStateD3D[RWD3D9_MAX_TEXTURE_STAGES][MAX_TEXTURESTAGESTATE_CACHE];
         ...
         FLOATASINT(LastMaterialAmbientSaturated.blue) != FLOATASINT(AmbientSaturated.blue) )
    {
            ...
            *((const RwUInt32 *)color) != 0xffffffff)
        {
            ...
            if (flags & rxGEOMETRY_PRELIT)
            {
				...
				RwD3D9SetRenderState(D3DRS_DIFFUSEMATERIALSOURCE, D3DMCS_MATERIAL);			// gemani
