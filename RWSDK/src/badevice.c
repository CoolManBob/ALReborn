...
#if ( defined WIN32 ) && ( ( defined USE_EXTERNAL_LOCK_FUNCTION ) || ( defined USE_EXTERNAL_LOCK_FUNCTION_WITH_FILEINFO ) )
	// 외부 Lock 함수 지정
    if ( NULL != lockFuncs )
    {
    	RWSRCGLOBAL( CsInitLock ) = lockFuncs->CsInitLock;
    	RWSRCGLOBAL( CsLock ) = lockFuncs->CsLock;
    	RWSRCGLOBAL( CsUnlock ) = lockFuncs->CsUnlock;
    	RWSRCGLOBAL( CsDelLock ) = lockFuncs->CsDelLock;
   	}
#endif // ( defined WIN32 ) && ( ( defined USE_EXTERNAL_LOCK_FUNCTION ) || ( defined USE_EXTERNAL_LOCK_FUNCTION_WITH_FILEINFO ) )

#if ( defined RWDEBUG )
	RWSRCGLOBAL( debugMessage ) = 0;
	RWSRCGLOBAL( debugMainThreadId ) = 0;
#endif
//@}
