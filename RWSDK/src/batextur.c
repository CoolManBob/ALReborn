...
/* 2006. 4. 5. nonstopdj
 * edit RWSTRNSTORE macro. has RWERROR messsge.
 */
#define RWSTRNSTORENOERR(_dst, _src)                          \
MACRO_START                                                   \
{                                                             \
    const size_t n = sizeof((_dst))/sizeof((_dst)[0]);        \
                                                              \
...
#if defined WIN32
// Critical Section for dirtyFrameList and all frames
static CRITICAL_SECTION _csTexture;
#else // defined WIN32
static void* _csTexture = NULL;
#endif // defined WIN32
                                 ...
                                 RwImage * sourceImage)
{
        ...
        (rwRASTERFORMATPAL4 | rwRASTERFORMATPAL8))
    {
        if (RwRasterGetFormat(raster) & rwRASTERFORMATPAL4)
        {
            ...
            {
                for (mipLevel = 0; mipLevel < totalMipLevels; ++mipLevel)
                {
                    if (mipArray[mipLevel] != sourceImage)
                    {
                        RwImageDestroy(mipArray[mipLevel]);
...
RwTexDictionaryCreate(void)
{
	...
	for(i=0;i<512;++i)
	{
		rwLinkListInitialize(&dict->texturesInDict[i]);
	}
	
	//. 2006. 2. 1. Nonstopdj
	dict->texture = ((RwTexture*)NULL);
                              ...
                              RwTextureCallBack fpCallBack, void *pData)
{
	...
	for(i=0;i<512;++i)
	{
		end = rwLinkListGetTerminator(&dict->texturesInDict[i]);
		cur = rwLinkListGetFirstLLLink(&dict->texturesInDict[i]);
...
RwTextureDestroy(RwTexture *texture)
{
	...
	//. 2006. 1. 17. Nonstopdj
	//. original texture reference count is zero, 
	//. if reference count is zero, then annihilate original texture.
	//. when reference count greater than zero, this function has no action.
    if (texture->refCount <= 0)
	{
#ifdef RWDEBUG
		/* access abnormal texture raster block */
		if(RWISBADREADPTR(texture->raster, sizeof(RwRaster*)))
		{
			RWERROR((E_RW_INSUFFICIENTRASTER));
			
			/* pointer force set zero */
			if (texture->raster)
				texture->raster = (RwRaster*)NULL;

			if(RWISBADREADPTR(texture->dict, sizeof(RwTexDictionary*)))
			{
				RWERROR((E_RW_INSUFFICIENTDIC));
...
#endif
		/* normality texture remove on system*/
...
RwTextureSetName(RwTexture *texture, const RwChar *name)
{
	...
	texDict = RwTextureGetDictionary (texture);
	RwTexDictionaryRemoveTexture(texture);

	/* 2006. 4. 5. nonstopdj
	 * edit RWSTRNSTORE macro.
	 */
	RWSTRNSTORENOERR(texture->name, name);
...
{
	char*	pTextureName = RwTextureGetName(texture);
	int		iTextureNameLength = strlen(pTextureName);
	int		i;
    ...
    /* Check if the texture's name is unique in the texture dictionary. */
    {
        if (RwTexDictionaryFindNamedTexture(dict, texture->name))
        {
            ...
            rwsprintf(msg, RWSTRING("Texture name \"%s\" already exist in the dictionary."),
                texture->name);
	...
	// Key값 가져오기
	for(i=0;i<iTextureNameLength;++i)
	{
		keyval += pTextureName[i];
	}
	keyval %= 512;
	...
	//. 2006. 2. 1. Nonstopdj
	//. current texture linked at dictionary
	dict->texture = texture;

    texture->dict = dict;

    rwLinkListAddLLLink(&dict->texturesInDict[keyval], &texture->lInDictionary);
...
RwTexDictionaryFindNamedTexture(RwTexDictionary *dict, const RwChar *name)
{
    RwTexture          *result;
    RwLLLink           *cur, *end;
	int		iTextureNameLength = strlen(name);
	int		i;
	unsigned int keyval = 0;
    ...
    cur = rwLinkListGetFirstLLLink(&dict->texturesInDict[keyval]);
    end = rwLinkListGetTerminator(&dict->texturesInDict[keyval]);
 ...
 **/
RwTexture   *RwTextureDictionaryGetLastTexture(RwTexDictionary *dict)
{
	RWAPIFUNCTION(RWSTRING("RwTextureDictionaryGetLastTexture"));
    RWASSERT(dict);
    RWASSERTISTYPE(dict, rwTEXDICTIONARY);

	if(RWISBADREADPTR(dict->texture, sizeof(RwTexture*)))
		/* dict->texture is bad pointer */
		RWRETURN((RwTexture *)NULL);
	else
		RWRETURN(dict->texture);
}
...
RwTextureRead(const RwChar *name, const RwChar *maskName)
{
	...
	/* 2006. 3. 6. Nonstopdj		*/
	/* change Texture unlock position  */
	CS_TEXTURE_UNLOCK();
