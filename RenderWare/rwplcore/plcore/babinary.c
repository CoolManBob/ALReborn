        ...
        if (readType == type)
        {
            ...
            if (readVersion < rwLIBRARYBASEVERSION)
            {
                ...
                RWMESSAGE((RWSTRING("Old version 0x%x < rwLIBRARYBASEVERSION 0x%x, ")
                           RWSTRING("and should be converted with an older library"),
            ...
            else if (readVersion < rwLIBRARYNOWARNVERSION)
            {
                ...
                RWMESSAGE((RWSTRING("Old version data (0x%0x) encountered,")
                           RWSTRING(" content may be sub-optimal and may require")
                           RWSTRING(" conversion during streaming. Please see the")
                           RWSTRING(" UpdatingYourRWGAssets whitepaper for details"),
