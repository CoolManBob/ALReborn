    ...
    {    
          ...
          )
        {
            RWMESSAGE((RWSTRING("The current camera frame is orthonormal but ")
                RWSTRING("the matching matrix flags are not set. This could ")
                RWSTRING("result in performance loss as orthonormal ")
                RWSTRING("matrix operation optimization won't be used")));
