...
RwFrameCallBack		  g_FrameRemoveCallback = 0;
void*				  g_pDataClass = 0;
...
rwSetHierarchyRoot(RwFrame *frame, RwFrame *root)
{
	...
	// 호출하는 함수 : _rwFramePurgeClone, RwFrameAddChild, RwFrameAddChildNoUpdate, RwFrameRemoveChild
	//@} DDonSS

	/* 2006. 4. 10. nonstopdj
	 * blocking infinity loop. ㅠㅠ
	 */
	if(frame->child == root)
		RWRETURNVOID();
...
rwFrameCloneRecurse(RwFrame *oldFrame, RwFrame *newRoot)
{
	...
	//@{ 20050513 DDonSS : Threadsafe
	// Threadsafe 코드 추가하지 않음. Local Function이며, 호출하는 곳에서 처리
	// 호출하는 함수 : _rwFrameCloneAndLinkClones
	//@} DDonSS
...
rwFrameInternalInit(RwFrame *frame)
{
	...
	//@{ 20050513 DDonSS : Threadsafe
	// Threadsafe 코드 추가하지 않음. Local Function이며 생성시에만 호출됨
	// _rwFrameInit에서도 호출되나, _rwFrameInit은 static의 초기화를 위해 호출됨
	//@} DDonSS
...
RwFrameDestroy(RwFrame *frame)
{
	...
	//. 2006. 9. 13. nonstopdj
	if(g_FrameRemoveCallback && g_pDataClass)
		g_FrameRemoveCallback(frame, g_pDataClass);
...
rwFrameDestroyRecurse(RwFrame * frame)
{
    ...
    if (frame)
    {
		...
		//@{ Jaewon 20051212
		// Kill the die-hard threading bug!
		// Very good job, Mr.Cha.^^
...
RwFrameRemoveChild(RwFrame *child)
{
	...
	/* 2006. 3. 9. Nonstopdj */
	/* validation check.*/
	if(RwFrameGetParent(child) != NULL)
		curFrame = ((RwFrame *) RwFrameGetParent(child))->child;
	else
	{
    ...
    {
		while (curFrame && curFrame->next != child)
		{
...
{
	RWAPIFUNCTION(RWSTRING("RwFrameSetRemoveCallback"));
	g_FrameRemoveCallback	= RemoveCallback;
	g_pDataClass			= pData;
}
