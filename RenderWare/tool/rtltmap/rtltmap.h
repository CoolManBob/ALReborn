 ...
 *-  Module  :   rtltmap.h                                                  -*
 *-                                                                         -*
 *-  Purpose :   Lightmap toolkit public API                                -*
 *-                                                                         -*
 *===========================================================================*/
...
/* These three are used in post-processing summed area light contributions */
#define rtLTMAPRECIPLOG2 (1.4426950409f)
#define rtLTMAPMINLIGHTSUM (1.0000f)
#define rtLTMAPMAXLIGHTSUM (1024.0f)
...
/* This is used to generate lists of atomics/sectors for RtLtMapLightMapsCreate,
 * RtLtMapIlluminate and RtLtMapAreaLightGroupCreate */
typedef struct LtMapSessionInfo LtMapSessionInfo;
struct LtMapSessionInfo
{
    RwSList *localSectors;
...
/* These are per-area-light-triangle flags which are used during light
 * processing so that hierarchical visibility-checks can be done on lights
 * (back-face and ROI culling of objects/triangles w.r.t lights or lights
 * w.r.t triangles, etc, etc, etc). Every flag, when set, means "culled",
 * so only triangles with zero-bitfields are visible. The flags are updated
 *  */
enum RtLtMapALTriVisFlags
{
    rtLTMAPALTRIVISNAFLAG = 0,

    rtLTMAPALTRIVIS_BFC_WRTWORLDPOLY = 1, /* The current lightmapped polygon is
                                           * behind this area light triangle.
                                           * OR
                                           * This area light triangle is behind
                                           * the current lightmapped polygon. */

    rtLTMAPALTRIVISFORCENUMSIZEINT = 0x7FFFFFFF
};
typedef enum RtLtMapALTriVisFlags RtLtMapALTriVisFlags;

/* These are per-area-light-mesh flags which are used during light
 * processing so that hierarchical visibility-checks can be done on lights
 * (back-face and ROI culling of objects/triangles w.r.t lights or lights
 * w.r.t triangles, etc, etc, etc). Every flag, when set, means "culled",
 * so only area light meshes with zero-bitfields are visible. */
enum RtLtMapALMeshVisFlags
{
    rtLTMAPALMESHVISNAFLAG = 0,

    rtLTMAPALMESHVIS_TRI_OUTSIDE_ROI = 1, /* The current lightmapped polygon is outside
                                           * the R.O.I. of this area light mesh */
    rtLTMAPALMESHVIS_OBJ_NONVIS = 2,      /* The current lightmapped object is outside
                                           * the R.O.I. of this area light mesh
                                           * OR
                                           * The current lightmapped object is not visible
                                           * from this area light mesh (and vice versa) */

    rtLTMAPALMESHVISFORCENUMSIZEINT = 0x7FFFFFFF
};
typedef enum RtLtMapALMeshVisFlags RtLtMapALMeshVisFlags;
