...
_rwD3D9RasterLock(void *pixelsIn, void *rasterIn, RwInt32 accessMode)
{
        ...
        case rwRASTERTYPETEXTURE:
            {
				//@{ Jaewon 20041108
				// 3d texture support
				if (topRasExt->cube == TRUE+1)
				{
                    hr = IDirect3DVolumeTexture9_GetVolumeLevel((LPDIRECT3DVOLUMETEXTURE9)(topRasExt->texture), mipLevel, (LPDIRECT3DVOLUME9*)&(rasExt->lockedSurface));
				}
				else
				//@} Jaewon
                if (topRasExt->cube)
                ...
                if (SUCCEEDED(hr))
                {
                    if (topRaster == raster)
                    {
						...
						if (topRasExt->cube == TRUE+1)
						{
						hr = IDirect3DVolume9_LockBox((LPDIRECT3DVOLUME9)(rasExt->lockedSurface),
													  &lockedBox,
													  NULL,
													  flags);
						}
	...
	// 3d texture support
	if (topRasExt->cube == TRUE+1)
		raster->cpPixels = (RwUInt8 *)lockedBox.pBits;
	else
	...
	// 3d texture support
	if (topRasExt->cube == TRUE+1)
		raster->stride = lockedBox.SlicePitch;
	else
        ...
        case rwRASTERTYPETEXTURE:
            {
				//@{ Jaewon 20041108
				// 3d texture support	
				if(rasExt->cube == TRUE+1)
				{
                IDirect3DVolume9_UnlockBox((LPDIRECT3DVOLUME9)(rasExt->lockedSurface));

                IDirect3DVolume9_Release((LPDIRECT3DVOLUME9)(rasExt->lockedSurface));
				}
				else
				{
                ...
                case rwRASTERTYPETEXTURE:
                    {
                        ...
                        else if (IS_D3DFORMAT_ZBUFFER(rasExt->d3dFormat))
                        {
                            hr = DXCHECK(IDirect3DDevice9_CreateTexture(_RwD3DDevice,
                                                                        ...
                                                                        rasExt->d3dFormat,
                                                                        /*D3DPOOL_DEFAULT, gemani 2004.11.19*/
																		D3DPOOL_MANAGED,
                                                                        &(rasExt->texture),
                                                                        NULL));
                        }
...
// 3d texture support
RwBool
_rwD3D9CheckValidVolumeTextureFormat(RwInt32 format)
{
    const D3DFORMAT adapterFormat = _RwD3D9AdapterInformation.mode.Format;
    HRESULT         hr;

    RWFUNCTION(RWSTRING("_rwD3D9CheckValidVolumeTextureFormat"));
