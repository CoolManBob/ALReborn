        ...
        nativeRaster.height == raster->height)
    {
        ...
        do
        {
            for (i = 0; i < numMipLevels; i++)
            {
				...
				//@{ Jaewon 20051104
				// A stop-gap measure for the crash prevention in case of a currupted texture file.
                if (RwStreamRead(stream, (void *)&size, sizeof(RwUInt32)) != sizeof(RwUInt32)
				|| (stream->type==rwSTREAMMEMORY && size > stream->Type.memory.nSize-stream->Type.memory.position))
				//@} Jaewon
                {
                    RwRasterUnlock(raster);
