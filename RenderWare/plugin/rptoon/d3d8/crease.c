...
ScreenCameraSpaceProjectionProjectPoint(RwV2d *v0p,   /* projected x & y */
                                        ...
                                        RwUInt32 index)
{
    _rpToonVertexCache *vertexCached = &(_rpToonD3D8VertexCache[index]);
                    ...
                    const ScreenSpaceProjection *ssp)
{
    ...
    if (toonGeo->numCreaseEdges)
    {
        for (i = 0; i < toonGeo->numCreaseEdges; i++)
        {
            ...
            */
            if ( _rpToonD3D8IsFrontFace[toonGeo->edges[i].face[0]] ||
                 _rpToonD3D8IsFrontFace[toonGeo->edges[i].face[1]] )
            {
